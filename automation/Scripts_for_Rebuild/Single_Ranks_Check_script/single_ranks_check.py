# -*- coding: utf-8 -*-
# Checks if for each symptom API returns conditions in order according with their ranks
import requests
import urllib2
import re
import urllib
...
out_pass.write("Symptom\tNumber on " + \
           "Web\tNumber on API\t" + \
           "Content IDs found on API\tContent IDs found on Web\tAPI ranks\tResults\n")
		   
out_number_not_match.write("Symptom\tNumber on " + \
           "Web\tNumber on API\t" + \
           "Content IDs found on API\tContent IDs found on Web\tAPI ranks\tResults\n")

out_different_order.write("Symptom\tNumber on " + \
           "Web\tNumber on API\t" + \
...
out_contentID_not_match.write("Symptom\tNumber on " + \
           "Web\tNumber on API\t" + \
...
for symptom in f:
    ...
    if api_num == 'Error':
        ...
        print "API Error"
        num_api_err += 1
        api_err.write(symptom + '\t' + str(web_num) + '\t' \
                  + str(api_num) + '\n')
        continue
    ...
    if web_num == 'Error':
        ...
        print "Web Error"
        num_api_err += 1
        web_err.write(symptom + '\t' + str(web_num) + '\t' \
                  + str(api_num) + '\n')
        continue
