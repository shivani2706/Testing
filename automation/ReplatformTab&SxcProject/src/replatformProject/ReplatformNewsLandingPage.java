...
public class ReplatformNewsLandingPage {
	...
	public void ReplatformNewsTitleandDesc() throws InterruptedException {
...
try{
	...
	hamburger.click();
	WebElement hammenu = driver.findElement(By.xpath(".//*[@class='css-akp3jh']/div/div/section[4]/ul/li[6]/a"));
	hammenu.click();
	
	// Check title
		try {
			WebElement title = locateElement('s', ".css-d36iui .css-1euiufy h1");
		...
		try {
			WebElement description = locateElement('s', ".css-d36iui .css-1euiufy h2");
			isElementDisplayed(description, "Description below Title",
					messages);
		} catch (Exception e) {
			messages.add("Description missing below title on Replatform News Landing Page "
					+ driver.getCurrentUrl());
		}
}
catch (Exception e) {
...
public void ReplatformHorizontalLine() throws InterruptedException {
		List<String> messages = new ArrayList<String>();
		...
		try {
			...
			messages.add("Horizontal Line missing below Description on Replatform News Landing Page "
					+ e.toString());
		}
...
public void ReplatformNewsLandingarticlelinks() throws InterruptedException {
	List<String> messages = new ArrayList<String>();
	JavascriptExecutor jse = (JavascriptExecutor) driver;

	System.out
			.println("==================Centerwell News Article Links Test Case Start==================");
	...
	try {
		...
		for (WebElement readnextlist : readnextlists) {
			...
			checkHttpStatus(lnk, "Links from listed News Landing Page", messages);
			
		}
		System.out
				.println("All Links from Replatform News Landing Page are not broken "
						+ driver.getCurrentUrl());
	} catch (Exception e) {
		messages.add("Not found or problem for Articles on Replatform News Landing Page ");
	}
	...
	try {
		...
		System.out
				.println("Images for listed Article links are not missing from News Landing Page "
						+ driver.getCurrentUrl());
	} catch (Exception e) {
		messages.add("Not found or problem for Images on Replatform News Landing ");
	}
	
	
	finalAssert("Centerwell News Article Links", messages);
}
	...
	public void ReplatformNewsAds() throws InterruptedException {
		...
		try {
			...
			try {
				...
				messages.add("Problem with Top dlb Ad... on Replatform News Landing Page "
						+ e.toString());
			}
			...
			try {
				...
				for (int i = 1; i <= dcmrs.size()-10; i=i+2) {
					try {
						...
						String googleQueryId = dcmrBlock.findElement(
						...
						if (googleQueryId.isEmpty()) {
							messages.add("DCMR"
									+ j
									+ " Ad at right rail on Replatform News Landing Page is not fired.."
									+ driver.getCurrentUrl());
						} else {
							System.out
							.println("DCMR"	+ j	+ " ad is fired ...google query id is --> "+ googleQueryId);
							System.out
							.println("DCMR"
									+ j
						...
						messages.add("DCMR" + j
								+ " Ad is missing on Replatform News Landing Page ");
					}
					j++;
				}
			} catch (Exception e) {
			...
			messages.add("Replatform News Landing Page Ads test case exception ..."
					+ e.toString());
		}
